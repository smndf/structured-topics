	...
	private static Map<Integer, String[]> getStringGraphs(
		...
		for (Integer key : clusters.keySet()){
			...
			for(Integer node : nodesList){
				...
				if (imagesMap.containsKey(nodeString.toLowerCase())) imageURL = imagesMap.get(nodeAsKey);
				else if (imagesMap.containsKey(nodeString)) imageURL = imagesMap.get(nodeAsKey.toLowerCase());
				else imageURL = "http://serelex.org/image/"+nodeString.split(" ")[0];
